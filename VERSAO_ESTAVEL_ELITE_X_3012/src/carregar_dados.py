...
CAMINHO_BASE_PADRAO = RAIZ_PROJETO / "dados" / "mega_sena_historico.csv"
...
def _info_concurso_fallback() -> dict:
    return {
        "concurso": None,
        "data": None,
...
def buscar_info_concurso_atual() -> dict:
    fallback = _info_concurso_fallback()
    try:
        dados = _baixar_json_caixa(API_CAIXA_MEGA_SENA_URL)
    except ERROS_REDE_CAIXA:
        return fallback

    valor_estimado = dados.get("valorEstimadoProximoConcurso")
    proximo_concurso = dados.get("numeroConcursoProximo") or dados.get("numero")
    try:
        proximo_concurso = int(proximo_concurso) + (0 if dados.get("numeroConcursoProximo") else 1)
    except (TypeError, ValueError):
        proximo_concurso = None

    info = {
        "concurso": proximo_concurso,
        "data": dados.get("dataProximoConcurso"),
        "concurso_atual": dados.get("numero"),
        "proximo_concurso": proximo_concurso,
        "data_proximo_concurso": dados.get("dataProximoConcurso"),
        "premio_estimado": valor_estimado,
        "acumulou": bool(dados.get("acumulado")),
        "fonte": "CAIXA",
    }
    return {**fallback, **{chave: valor for chave, valor in info.items() if valor not in ("", None)}}
